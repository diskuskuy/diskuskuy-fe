...
export default function DiscussionGuide() {
  ...
  const [isLecturer, setIsLecturer] = useState(role == "lecturer" ? true : false);
              ...
              {discussionGuideConstants.map((object, i) => (
                <div
                  ...
                  style={{
                    ...
                    boxShadow: !checkboxDisabled(i) ? '0 25px 50px -12px rgb(0 0 0 / 0.25)' : '0 10px 15px -3px rgb(0 0 0 / 0.1), 0 4px 6px -4px rgb(0 0 0 / 0.1)',
                  }}
                  onClick={!checkboxDisabled(i) ? handleChangeCheckbox : () => {}}
                >
                  <h1 className="font-bold">{object.id}</h1>
                  <div className="flex flex-col">
                    <p className="font-bold">{object.title}</p>
                    <h1>{isLecturer} </h1>
                    <p dangerouslySetInnerHTML={{ __html: object.description }}></p>
                    {object.hints != null && (
                      <>
                        <p className="font-bold">Hints</p>
                        <p dangerouslySetInnerHTML={{ __html: object.hints }}></p>
                      </>
                    )}
                  </div>
                  <input type="checkbox" checked={i + 1 < discussionGuideData.state} disabled={checkboxDisabled(i)}/>
                </div>
              ))}
              <p>Bila ada yang dirasa membingungkan, jangan ragu untuk bertanya langsung pada thread diskusi ya :)</p>
            </div>
